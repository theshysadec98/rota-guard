# RotaGuard

RotaGuard là ứng dụng desktop Java dùng Swing và JDBC để quản lý nhân sự trên một bảng `staff`.
Chương trình hỗ trợ 5 thao tác cơ bản: hiển thị, thêm, cập nhật, xóa và reset dữ liệu trên giao diện `JFrame`.

## Yêu cầu

- JDK 21
- Docker và Docker Compose

## Cấu trúc chính

```text
rota-guard/
├── README.md
├── TECH.md
├── dev.sh
├── docker-compose.yml
└── be/
    ├── build.gradle.kts
    ├── sql/schema.sql
    ├── spotless/license-header.java
    └── src/main/java/com/rotaguard/jdbc/
```

## Bảng `staff`

| Cột | Kiểu | Ý nghĩa |
|-----|------|---------|
| `ma_nv` | `VARCHAR(20)` | Mã nhân viên, khóa chính |
| `ho_ten` | `VARCHAR(255)` | Họ tên |
| `khoa` | `VARCHAR(100)` | Khoa / đơn vị |
| `he_so` | `DOUBLE PRECISION` | Hệ số |
| `vai_tro` | `VARCHAR(20)` | `DOCTOR` hoặc `NURSE` |
| `sdt` | `VARCHAR(15)` | Số điện thoại |
| `email` | `VARCHAR(255)` | Email |
| `nam_kn` | `INT` | Năm kinh nghiệm |

File [schema.sql](/Users/hoang/my-app/rota-guard/be/sql/schema.sql:1) tạo bảng và seed sẵn 3 dòng dữ liệu mẫu.

## Cách chạy

Khởi động PostgreSQL:

```bash
./dev.sh up
```

Nạp lại schema:

```bash
./dev.sh schema
```

Chạy ứng dụng:

```bash
./dev.sh run
```

Build và format mã Java:

```bash
./dev.sh build
```

Dừng PostgreSQL:

```bash
./dev.sh down
```

## Sử dụng giao diện

1. Bấm `Hiển thị` để tải danh sách nhân sự.
2. Nhập đầy đủ thông tin rồi bấm `Thêm` để tạo mới.
3. Chọn một dòng trong bảng để đổ dữ liệu lên form.
4. Sửa thông tin và bấm `Cập nhật`.
5. Chọn nhân sự và bấm `Xóa` để xóa.
6. Bấm `Reset` để đưa form về giá trị mặc định.

## Cấu hình kết nối cơ sở dữ liệu

File mặc định: `be/src/main/resources/db.properties`

```properties
db.url=jdbc:postgresql://localhost:5432/rotaguard
db.username=rotaguard
db.password=rotaguard
```

Có thể ghi đè bằng biến môi trường `ROTAGUARD_DB_URL`, `ROTAGUARD_DB_USERNAME`, `ROTAGUARD_DB_PASSWORD`.
