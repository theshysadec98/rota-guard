...
:try_local_mysql
mysql --protocol=tcp -h 127.0.0.1 -P 3306 -u staffmanager -pstaffmanager -e "SELECT 1" >nul 2>&1
...
mysql --protocol=tcp -h 127.0.0.1 -P 3306 -u staffmanager -pstaffmanager -e "CREATE DATABASE IF NOT EXISTS staffmanager;" >nul 2>&1
if errorlevel 1 exit /b 1
type "%ROOT%be\sql\schema.sql" | mysql --protocol=tcp -h 127.0.0.1 -P 3306 -u staffmanager -pstaffmanager staffmanager
if errorlevel 1 exit /b 1
...
echo   dev.bat
echo.
echo Uu tien MySQL local voi tai khoan staffmanager/staffmanager.
